...
def export_nominalization_sebset(file_name):
    ...
    with open(file_name) as fin:
        for line in fin.readlines():
            ...
            noun, pos = posTaggedSent[predicate_idx]

            # is this noun a candidate for being a deverbal nominalization?
            if pos in cand.COMMON_NOUNS_POS and cand.is_candidate_noun(noun):
...
def export_verbal_nomtype_sebset(file_name):
    ...
    with open(file_name) as fin:
        for line in fin.readlines():
            ...
            if pos in cand.COMMON_NOUNS_POS and nom_type and nom_type not in exclude:
                nom_lines.append(line)
    # print info
