...
def split_by_pos(file_name):
    ...
    with open(file_name) as fin:
        for line in fin.readlines():
            ...
            doc = spacy_nlp(sent)
            predicate_pos = doc[predicate_idx].pos_

            if predicate_pos=='NOUN':
                noun_lines.append(line)
            elif predicate_pos=='VERB':
                verb_lines.append(line)
            else:
                other_lines.append(line)
    # print info
    print(f"splitting {file_name} by POS: found {len(verb_lines)} verbs, {len(noun_lines)} nouns, and {len(other_lines)} others.")
    # write to files
